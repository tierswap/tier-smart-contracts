...
import "./interfaces/ITierRouter02.sol";
...
contract TierRouter is ITierRouter02 {
	...
	function _addLiquidity(
		...
		(uint256 reserveA, uint256 reserveB) = TierLibrary.getReserves(
			factory,
			tokenA,
			tokenB
		);
		if (reserveA == 0 && reserveB == 0) {
			...
			uint256 amountBOptimal = TierLibrary.quote(
				amountADesired,
				reserveA,
				reserveB
			);
			if (amountBOptimal <= amountBDesired) {
				...
				uint256 amountAOptimal = TierLibrary.quote(
					amountBDesired,
					reserveB,
					reserveA
				);
		...
		returns (uint256 amountA, uint256 amountB)
	{
		...
		(uint256 amount0, uint256 amount1) = ITierPair(pair).burn(to);
	...
	function _swapSupportingFeeOnTransferTokens(
		...
		for (uint256 i; i < path.length - 1; i++) {
			(address input, address output) = (path[i], path[i + 1]);
			(address token0, ) = TierLibrary.sortTokens(input, output);
			ITierPair pair = ITierPair(
				TierLibrary.pairFor(factory, input, output)
			);
			uint256 amountInput;
			uint256 amountOutput;
			{
				...
				amountOutput = TierLibrary.getAmountOut(
					amountInput,
					reserveInput,
					reserveOutput
				);
	...
	function getAmountIn(
		...
		return TierLibrary.getAmountIn(amountOut, reserveIn, reserveOut);
	}
